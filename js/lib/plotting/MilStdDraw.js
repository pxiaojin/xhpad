...
MilStd.EnumMilstdType = {
    ...
    BezierCold: 'BezierCold',                               //冷锋
    BezierHot: 'BezierHot',                                 //暖锋
    BezierStatic: 'BezierStatic',                            //静止锋
    Circle: 'Circle'                                         //圆形
};
 ...
 * @returns {*}
 */
MilStd.commonFun.getBezierColdPoints = function (points, n) {
    var geometryArr = new Array();
    if ((points.length <= 2) || (points.length <= n)) {
        var mulLin = new ol.geom.Point(points);
        geometryArr.push(mulLin);
        return geometryArr;
    }
    var list = smoothLine(points, 0.1, 0.24);
    ...
    for(var k=0;k<list.length;k++){
        ...
        k+=4*size;
        if(k<list.length){
            var start = list[k-1*size];
            var mid = list[k-2*size];
            var end = list[k-3*size];

            var num = 2;
            var theta = 180;
            for(var i=0;i<num+1;i++){
                var beta = (theta / num * i) / 180 * Math.PI;
                var x = mid[0] - start[0];
                var y = mid[1] - start[1];
                var xx = x * Math.cos(beta) + y * Math.sin(beta);
                var yy = y * Math.cos(beta) - x * Math.sin(beta);
                var resxx = mid[0] + xx;
                var resyy = mid[1] + yy;
                var point = [resxx,resyy];
                line.push(point);
            }

            //通过两点求第三点的坐标
            // var scale = Math.atan((mid[1]-end[1])/(end[0]-mid[0]));
            // var aa = 90 - scale/Math.PI * 180;
            // var l = 3*Math.sqrt(Math.pow(mid[1]-end[1],2)+Math.pow(end[0]-mid[0],2));
            // var point = [mid[0]+l*Math.cos(aa/180*Math.PI), mid[1]+l*Math.sin(aa/180*Math.PI)];
            // var line = [start,mid,end,point,start];

            lineLists.push(line);
        }
    }

    var mulLin = new ol.geom.MultiPolygon([lineLists]);
    var trianglePro = {
        name:'triangleCold'
    };
    mulLin.setProperties(trianglePro);
    geometryArr.push(mulLin);

    var lineString = new ol.geom.LineString(list);
    var lineStringPro = {
        name:'lineStringCold'
    };
    lineString.setProperties(lineStringPro);
    geometryArr.push(lineString);
 ...
 * @returns {*}
 */
MilStd.commonFun.getBezierHotPoints = function (points, n) {
    var geometryArr = new Array();
    if ((points.length <= 2) || (points.length <= n)) {
        var mulLin = new ol.geom.Point(points);
        geometryArr.push(mulLin);
    ...
    mulLin.setProperties(circlePro);
    geometryArr.push(mulLin);
...
/**
 * 获取静止锋的曲线和半圆区域的矩形和三角区域的矩形
 * @param points
 * @param n
 * @returns {*}
 */
MilStd.commonFun.getBezierStaticPoints = function (points, n) {
    var geometryArr = new Array();
    if ((points.length <= 2) || (points.length <= n)) {
        var mulLin = new ol.geom.Point(points);
        geometryArr.push(mulLin);
    ...
    for(var j=0;j<list.length;j++){
        ...
        j+=5*size;
        if(j<list.length){
            var start = list[j-5*size];
            var mid = list[j-4*size];
            var end = list[j-3*size];
            var num = 2;
            var theta = 180;
            for(var i=0;i<num+1;i++){
                var beta = (theta / num * i) / 180 * Math.PI;
            ...
            triangleLists.push(line);
        }
    }
    var circleLin = new ol.geom.MultiPolygon([circleLists]);
    var circlePro = {
        name:'circleStatic'
    };
    circleLin.setProperties(circlePro);
    geometryArr.push(circleLin);

    var triangleLin = new ol.geom.MultiPolygon([triangleLists]);
    var trianglePro = {
        name:'triangleStatic'
    };
    triangleLin.setProperties(trianglePro);
    geometryArr.push(triangleLin);

    var lineString = new ol.geom.LineString(list);
    var lineStringPro = {
        name:'lineStringStatic'
    };
    lineString.setProperties(lineStringPro);
    geometryArr.push(lineString);

    // var mulPoints = new ol.geom.MultiPoint(points);
    // var mulPointsPro = {
    //     name:'mulPointsStatic'
    // };
    // mulPoints.setProperties(mulPointsPro);
    // geometryArr.push(mulPoints);

    // var delPoint = new ol.geom.Point(points[points.length-1]);
    //
    // var delPointpro = {
    //   name:"delPointStatic"
    // };
    // delPoint.setProperties(delPointpro);
    // geometryArr.push(delPoint);
    ...
    if (arg != null && arg.length >= 2) {
        ...
        switch (compassType) {
            ...
            case "Vane":                     //风向标指北针
                ...
                break;
            case "Circle":
                parseDots = this.GetCircleDots(dots);
                break;
        }
...
/**
 * Method: GetCircleDots
 * 根据控制点数组，获取组成圆形的二维点数组
 * Parameters:
 * arg1-{Array.<ol.Coordinate>:控制点数组}
 * Returns:
 * locArr-{Array.<Array.<ol.Coordinate>>：二维点数组，圆形尖角由4部分组成，每一部分的用一个点数组存储}
 */
MilStd.Compass.GetCircleDots = function (arg1) {
    if (arg1 == null || arg1.length < 2) {
        return null;
    }
...
MilStd.Bezier.getBezierFengFromVert = function (arg, bazierType) {

    var geomArr  = null;
    if(bazierType == "BezierCold"){
        geomArr = MilStd.commonFun.getBezierColdPoints(arg,2);
    }else if(bazierType == "BezierHot"){
        geomArr = MilStd.commonFun.getBezierHotPoints(arg,2);
    }else if(bazierType == "BezierStatic"){
        geomArr = MilStd.commonFun.getBezierStaticPoints(arg,2);
    }
    return geomArr;
};
...
MilStd.MilStdGeomtry = function (verticePnts, miltype, milStdParams, opt_options) {
    ...
    if (this.milStdType == MilStd.EnumMilstdType.TriangleFlag || this.milStdType == MilStd.EnumMilstdType.RectFlag ||
        ...
        this.milStdType == MilStd.EnumMilstdType.AssemblyArea ||this.milStdType == MilStd.EnumMilstdType.Circle) {
...
MilStd.MilStdGeomtry.prototype.Create = function () {
    ...
    switch (this.milStdType) {
        ...
        case MilStd.EnumMilstdType.Circle:
        ...
        case MilStd.EnumMilstdType.Vane:
            ...
            this.setGeometriesArray(geomArr);
            break;
        case MilStd.EnumMilstdType.BezierCold:
        case MilStd.EnumMilstdType.BezierHot:
        case MilStd.EnumMilstdType.BezierStatic:
            var geomArr = MilStd.Compass.getBezierFengFromVert(this.vertices, this.milStdType);
            this.setGeometriesArray(geomArr);
            break;
...
MilStd.MilStdGeomtry.prototype.Update = function (vertices, isMouseMove) {
    ...
    switch (this.milStdType) {
        ...
        case MilStd.EnumMilstdType.BezierHot:
        case MilStd.EnumMilstdType.BezierStatic:
            var geomArr = MilStd.Bezier.getBezierFengFromVert(vertices, this.milStdType);
            this.setGeometriesArray(geomArr);
            break;
        case MilStd.EnumMilstdType.Bezier:
        case MilStd.EnumMilstdType.BezierLine:
...
goog.inherits(MilStd.event.MilStdDrawEvent, ol.events.Event);
...
MilStd.tool.MilStdDrawTool.prototype.activate = function (milType, milStdParams, name,index) {
    this.deactivate();
    this.ShieldDBClickZoomEvent(this.map);   //屏蔽DoubleClickZoom的监听
    ol.events.listen(this.mapViewport, ol.events.EventType.CLICK, this.drawStartHandle, this, false);
    this.milStdType = milType;
    this.milStdParams = milStdParams;
  
    // this.featureName = goog.isDefAndNotNull(name) ? name : "draw";

    this.featureName = milType+index;
    console.info(this.featureName);
    var layerName = {
        name:this.featureName
    }
    this.featureOverLay.setProperties(layerName);
...
MilStd.tool.MilStdDrawTool.prototype.drawStartHandle = function (e) {
    ...
    // ol.events.unlisten(this.mapViewport, ol.events.EventType.CLICK, this.drawStartHandle, false ,this);
    ol.events.unlisten(this.mapViewport, ol.events.EventType.CLICK, this.drawStartHandle, this);
    ol.events.listen(this.mapViewport, ol.events.EventType.CLICK, this.drawContinueHandle, this, false );
    ol.events.listen(this.mapViewport, ol.events.EventType.DBLCLICK, this.drawEndHandle, this, false );
    ol.events.listen(this.mapViewport, ol.events.EventType.MOUSEMOVE, this.mouseMoveHandle, this,  false );
};
...
MilStd.tool.MilStdDrawTool.prototype.disconnectEventHandlers = function () {
    ...
    $('.huiZhi .bottomT ul li ').removeClass('current');
};

...
// define(function(require){
	...
	function smoothLine(arrs,interval,distance){
		//111319.49079327358是一度约等于几米

		interval = interval*111319.49079327358;
		distance = distance*111319.49079327358;
		...
		if(points) {
			...
			for (var i = 0; i < ps.length; i++) {
				xs.push(validParseFloat(ps[i][0]||ps[i].x));
				ys.push(validParseFloat(ps[i][1]||ps[i].y));
			}

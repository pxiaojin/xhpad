define(['Controller/Http','Function/numberFC/FloatSelector','Function/wea_warn/warn_drawgeojson'], function(http, floatSelector,draw) {
    ...
    function init(){
        key = 'TS';
        button = $('#weather_warning_select .warnThunderstorm');
        ...
        legend = '<p class="tuCengP">雷暴落区</p>' + 
                '<img src="img/warn_legend/ts.png" alt="" />' +
                '<p>' +
                    '<span>单位</span>' +
                    '<span>%</span>' +
                '</p>';
        item = XHW.C.layerC.createItem('雷暴落区', legend, function(){
            close();
        })
    }
    ...
    function requestData(){
        ...
        // };
        var url = baseurl + '/gfs/polygondata?level=9999&elem=' + key + timeparam;
        $.ajax({
            url: appendInfoToURL(url),
            dataType:'json',
            success:function(json){          
                if(json.status_code != 0){
                    item.htmlLayer = ' 雷暴落区(无数据)';
                ...
                if (data) {
                    ...
                    item.htmlLayer = Number(month) + '月' + Number(day) + '日 ' + hour + ':00 ' + ' 雷暴落区';
                    XHW.C.layerC.updateLayerData(key, item);
                }
                $('#showTuLi .tuLiTS').show();
            },
            error: function(error){
                time = XHW.silderTime;
                item.htmlLayer = Number(time.month) + '月' + Number(time.day) + '日 ' + time.hour + ':00 ' + ' 雷暴落区' + '(无数据)';
                XHW.C.layerC.updateLayerData(key, item);
                remove();
            }
        });

...
define(['Controller/DataFormat', 'Function/numberFC/FloatSelector'], function(format, floatSelector) {
    ...
    function getData(){    //TODO, 之后更改写法（传递时间，elem固定
        ...
        XHW.C.http.Http(XHW.C.http.ecmfUrlNew, '/fio/areadata?elem=WINDY' + param, function(json, time){
            data = json;
            XHW.C.http.Http(XHW.C.http.ecmfUrlNew, '/fio/areadata?elem=WINDX' + param, function(json){
                dataS = json.vals;
                drawSeaWind();
            })
        });
    ...
    function showWindStream(Wdata){
        ...
        //step3 创建风流线对象
        var layer = new seaWindLayer(data, {
        ...
        var elem = 'sea_9999_9999_SEAWIND';

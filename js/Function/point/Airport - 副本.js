...
define(['Function/point/AirportInfo', 'Controller/DataFormat'], function(airportInfo, format) {
    ...
    function drawStationMarkers(){
        ...
        var markers = [];
        for(var key in data) {
            if(data[key] > level) continue;     //数据层级大于要显示的层级，不显示
            ...
            if (!url[1] || url[1] == 0 || url[1] == 999) { // 无跑道方向
    ...
    function getWeaColor(wea,vis){ 
        if(wea == undefined) {
            ...
            if(wea.indexOf('雾') == 0 || wea.indexOf('霾') != -1 && vis <= 1000){
                return 'yellow';
            }else if(wea.indexOf('沙') != -1 || wea.indexOf('尘') != -1){
                return 'brown';

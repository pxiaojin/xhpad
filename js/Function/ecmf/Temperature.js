...
define(['Controller/DataFormat'], function(format) {
    ...
    function getData(){    //TODO, 之后更改写法（传递时间，elem固定
        ...
        // http://weather.xinhong.net/ecmf/isolinedata?level=0600&elem=TT&year=2019&month=01&day=07&hour=09
        XHW.C.http.Http(XHW.C.http.ecmfUrl, '/ecmf/isolinedata?elem=TT' + param, function(data, time){
            time = format.jsonDate(time);
            item.htmlLayer = //time[0] + ' 发布 ' + 
                    time[1] + ' ' + select.find("option:selected").text() + '气温';

...
define(['Controller/DataFormat'], function(format) {
    ...
    function init(){
        ...
        key = 'ecmf_turb';
        legend = '<p class="tuCengP">颠簸</p>' + 
                '<img src="img/legend/turb.png" alt="" />';
    ...
    function getData(){    //TODO, 之后更改写法（传递时间，elem固定
        ...
        XHW.C.http.Http(XHW.C.http.ecmfUrl, '/ecmf/isosurfacedata?elem=TURB' + param, function(data, time){
            time = format.jsonDate(time);
            item.htmlLayer = // time[0] + ' 发布 ' +
                     time[1] + ' ' + select.find("option:selected").text() + '颠簸';
